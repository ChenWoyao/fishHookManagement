...
let saleOrderList = genSaleOrderList(200)
...
function getRule(req, res, u) {
  ...
  let tableDataSource = saleOrderList.slice()
...
function postRule(req, res, u, b) {
  ...
  const { key, type, url, sale_num, sale_price, total_sale_price, make_time, username, method } = body;
  ...
  switch (method) {
    ...
    case 'delete':
      (() => {
        saleOrderList = saleOrderList.filter((item) => item.key !== key);
    ...
    case 'update':
      (() => {
        let newRule = {};
        saleOrderList = saleOrderList.map((item) => {
          if (item.key === key) {
            let make_time = new Date()
            newRule = { ...item, make_time, sale_num, sale_price, total_sale_price };
            return newRule;
          }
...
export default {
  'GET /api/saleOrderQuery': getRule,
  'POST /api/saleOrderQuery': postRule,
}
